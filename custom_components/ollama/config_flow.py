...
def ollama_config_option_schema(
    ...
    # Add web search configuration options for conversation agents
    if subentry_type == "conversation":
        schema.update(
            {
                vol.Optional(
                    CONF_WEB_SEARCH_ENABLED,
                    description={
                        "suggested_value": options.get(CONF_WEB_SEARCH_ENABLED, False)
                    },
                ): BooleanSelector(),
                vol.Optional(
                    CONF_SEARCH_PROVIDER,
                    description={
                        "suggested_value": options.get(CONF_SEARCH_PROVIDER, DEFAULT_SEARCH_PROVIDER)
                    },
                ): SelectSelector(
                    SelectSelectorConfig(
                        options=[
                            SelectOptionDict(label=label, value=value)
                            for value, label in SEARCH_PROVIDERS.items()
                        ]
                    )
                ),
                vol.Optional(
                    CONF_SEARCH_URL,
                    description={
                        "suggested_value": options.get(CONF_SEARCH_URL, DEFAULT_SEARCH_URL)
                    },
                ): TextSelector(TextSelectorConfig(type=TextSelectorType.URL)),
                vol.Optional(
                    CONF_SEARCH_RESULTS_COUNT,
                    description={
                        "suggested_value": options.get(CONF_SEARCH_RESULTS_COUNT, DEFAULT_SEARCH_RESULTS_COUNT)
                    },
                ): NumberSelector(
                    NumberSelectorConfig(
